var lessons_table = new Vue({
        ...
        data: {
            ...
            highestCredit: undefined,
            limitCredit: undefined,
            searchTeacherName: '',
            // searchByTeacherNameLessons: [],
            searchLessonsByTeacherNameResult: ''
        },
        mounted: function () {
            var vm = this;
        ...
        methods: {
            ...
            addCreditTrigger: function () {
                ...
                $.ajax('lessons/addCreditTrigger', {
                    ...
                    if (data === "success") {
                        ...
                        vm.limitCredit = vm.highestCredit;
                    } else {
                        console.log("wrong");
                    }
                });
            ...
            insertLessonFunc: function () {
                ...
                this.lessons.forEach(function (element) {
                    if (vm.insertLesson.lessonNum === element.lessonNum) {
                        flag = true;
                    }
                });
                ...
                if (!flag) {
                    $.ajax('lessons/insertLesson', {
                        ...
                        if (data === "success") {
                            if (vm.insertLesson.credit > vm.limitCredit) {
                                swal("学分大于最高限制！", "你已成功添加所选课程数据，但学分将被更改为最高限制值");
                                vm.insertLesson.credit = vm.limitCredit;
                            } else {
            ...
            searchLessonFunc: function () {
                ...
                vm.searchResult = '没有找到要求搜索的内容';
            }
            ,

            searchLessonsByTeacherName: function () {

                var vm = this;
                this.lessons.forEach(function (element) {
                    if (element.teacher.teacherName === vm.searchTeacherName) {
                        vm.searchLessonsByTeacherNameResult += "课程号：" + element.lessonNum + "  课程名：" + element.lessonName + "\n";
                    }
                });

                if (vm.searchLessonsByTeacherNameResult === '') {
                    vm.searchLessonsByTeacherNameResult = '没有找到要求搜索的内容';
                    swal("数据查找失败！", vm.searchLessonsByTeacherNameResult, "error");
                } else {
                    swal("所授课程信息", vm.searchLessonsByTeacherNameResult);
                }

                vm.searchLessonsByTeacherNameResult = '';
            }
        }
    })
    ;